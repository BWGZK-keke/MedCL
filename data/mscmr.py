...
def make_transforms(image_set):
    ...
    if image_set == 'val':
        return T.Compose([
            ...
            T.NoRescale(),
            T.CenterPadOrCropToSize([212,212]),
            # T.RandomColorJitter(),
            normalize])
...
def build(image_set, args):
    root = Path('/data/kzhang99/datasets/' + args.dataset)
    ...
    if image_set == "train":
        ...
        for task, value in args.tasks.items():
            ...
            lab_paths = lab_paths*3
            dataset_train = seg_train(img_paths, lab_paths, lab_values, transforms=make_transforms(image_set))
