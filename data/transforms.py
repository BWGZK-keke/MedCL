...
def pad(image, target, region):
    ...
    ny = int(ny*scaler)
    if abs(x-nx) >0:
        x_s_index = (x-nx)/abs(x-nx)
        x_s = int(x_s_index*random.randint(0,int(abs(x-nx))))
    else:
        x_s = 0
        
    if abs(y-ny) >0:
        y_s_index = (y-ny)/abs(y-ny)
        y_s = int(y_s_index*random.randint(0,int(abs(y-ny))))
    else:
        y_s = 0
    # y_s_index = (y-ny)/abs(y-ny)
    # y_s = y_s_index*random.randint(0,abs(y-ny))
    x_c = -x_s
    y_c = -y_s
...
def center_pad(image, target, region):
    z,x,y = image.shape
    nx,ny = region
    x_s = (x-nx)//2
    y_s = (y-ny)//2
...
def resize(image, target, size):
    rescaled_width = size[0]
    rescaled_height = size[1]
    rescaled_size = [rescaled_width, rescaled_height]
    image = image.copy()
    image = torch.from_numpy(image)
    rescaled_image = F.resize(image, rescaled_size, interpolation = PIL.Image.Resampling.NEAREST)
    ...
    if "masks" in target:
        ...
        mask = F.resize(interpolate_mask, rescaled_size, interpolation = PIL.Image.Resampling.NEAREST)
...
def resize_random(image, target, size):
    min_scale = size[0]
    max_scale = size[1]
...
class RandomResize(object):
    ...
    def __call__(self, img, target=None):
        return resize_random(img, target, self.size)
...
class CenterPadOrCropToSize(object):
    def __init__(self, size):
        self.size = size

    def __call__(self, img, target):
        crop_height, crop_width = self.size
        padded_img, padded_target = center_pad(img, target, (crop_height, crop_width))
        return padded_img, padded_target
...
class RandomRotate(object):
    ...
    def __call__(self, img, target):
        ...
        img = img.copy()
        rotated_img = F.rotate(torch.from_numpy(img), angle1, PIL.Image.Resampling.NEAREST, self.expand, self.center) 
        rotated_img = rotated_img.numpy()
        mask = target['masks']
        mask = mask.copy()
        rotated_mask = F.rotate(torch.from_numpy(mask), angle1, PIL.Image.Resampling.NEAREST, self.expand, self.center)  
    ...
    #         img = img.copy()
    #         rotated_img = F.rotate(torch.from_numpy(img), angle1, PIL.Image.Resampling.NEAREST, self.expand, self.center)
    #         rotated_img_aux = F.rotate(torch.from_numpy(img), angle2, PIL.Image.Resampling.NEAREST, self.expand, self.center)
    ...
    #         mask = mask.copy()
    #         rotated_mask = F.rotate(torch.from_numpy(mask), angle1, PIL.Image.Resampling.NEAREST, self.expand, self.center)
    #         rotated_mask_aux = F.rotate(torch.from_numpy(mask), angle2, PIL.Image.Resampling.NEAREST, self.expand, self.center)
    #         rotated_mask_aux = rotated_mask_aux.numpy()
    #         rotated_mask = rotated_mask.numpy()
    #         if random.random() < 0.5:
    ...
    #             indicator = np.zeros_like(rotated_img)
    ...
    #             rotated_mask_onehot = one_hot(rotated_mask)
    #             rotated_mask_aux_onehot = one_hot(rotated_mask_aux)
    ...
    #             mix_label = mix_label.argmax(-1).astype(np.uint8)
    #         else:
    #             mix_img = img
    #             mix_label = mask

    #     target["masks"] = mix_label
