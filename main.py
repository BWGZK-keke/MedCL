...
def get_args_parser():
    ...
    parser.add_argument('--batch_size', default=32, type=int)#192#64
    ...
    parser.add_argument('--dataset', default='scribble', type=str,
                        help='multi-sequence CMR segmentation dataset')
    parser.add_argument('--output_dir', default='/data/kzhang99/UNet_experiments/MSCMR_MedCL_25_drop_half_shuffle_anatomy_v2/',
                        help='path where to save, empty for no saving')
    parser.add_argument('--device', default='cuda', type=str,
                        help='device to use for training / testing')
    parser.add_argument('--GPU_ids', type=str, default = '0,1,2,3,4,5,6,7', help = 'Ids of GPUs')    
    parser.add_argument('--seed', default=42, type=int)
    parser.add_argument('--resume', default='')
    # parser.add_argument('--resume', default='/data/kzhang99/UNet_experiments/MSCMR_MedCL_25_drop_half_shuffle_anatomy/0.886_selected_checkpoint.pth', help='resume from checkpoint')#/data/zhangke/experiments_resume/self_35/sota_checkpoint.pth
...
def main(args):
    ...
    dataloader_val_dict = {
        k : DataLoader(v1, 1, sampler=v2, drop_last=False, collate_fn=utils.collate_fn, num_workers=args.num_workers) 
        for (k, v1), v2 in zip(dataset_val_dict.items(), sampler_val_dict.values())
        }

    if args.frozen_weights is not None:
        checkpoint = torch.load(args.frozen_weights, map_location='cpu')
        model_without_ddp.whst.load_state_dict(checkpoint['model'], strict=False)
    ...
    for epoch in range(args.start_epoch, args.epochs):
        ...
        train_stats = train_one_epoch(model, criterion, dataloader_train_dict, optimizer, device, epoch, estimate_alpha)
        ...
        dice_score = test_stats["Avg"]#test_df["dice"].mean()
