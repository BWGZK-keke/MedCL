...
class MSCMR(nn.Module):
    def __init__(self, args, freeze_whst=False):
        ...
        # self.prototype = nn.Linear(212 * 212, 128, bias=False)
        self.net_anatomy = FeedForward(212*212, 1024, output_dim=128)
        self.net_individual = FeedForward(212*212, 1024, output_dim=128)
        self.Norm1d = nn.BatchNorm1d(5,eps=1e-3, momentum=0.01)
        self.active = torch.nn.Softmax(dim=-1)


    def forward(self, samples: torch.Tensor, task):
        seg_masks, norm_anatomy, norm_individual = self.UNet(samples)
        B = seg_masks.shape[0]
        c = seg_masks.shape[1]
        seg_flat = seg_masks.reshape((B,c,-1))
        seg_all_slice = seg_flat[:,1:c].sum(1, keepdim=True)
        seg_flat = torch.cat([seg_flat, seg_all_slice],1)
        anatomy_code = self.net_anatomy(seg_flat)
        individual_code = self.net_individual(seg_flat)

        # x = self.prototype(seg_flat)
        # x = self.Norm1d(x)
        # x = self.active(x)
        out = {"pred_masks": seg_masks, "anatomy_code": anatomy_code, "individual_code": individual_code}
        return out
